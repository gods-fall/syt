<template>
  <div class="reister">
    <div class="top">
      <div class="title">{{ hostDetail.hospitalInfo.hospital?.hosname }}</div>
      <div class="level">
        <svg class="icon" height="16" p-id="1502" t="1705319774821" version="1.1"
          ...
          <path
              ...
              fill="#5D5D5D" p-id="1505"></path>
        </svg>

        <span>{{ hostDetail.hospitalInfo.hospital?.param.hostypeString }}</span>
      </div>
    </div>
    <div class="content">
      <div class="left">
        <img :src="`data:image/jpeg;base64,${hostDetail.hospitalInfo.hospital?.logoData}`" alt="">
      </div>
      <div class="right">
        <div>挂号规则</div>
        <div class="time">预约周期:{{ hostDetail.hospitalInfo.bookingRule?.cycle }}天
          放号时间:{{ hostDetail.hospitalInfo.bookingRule?.releaseTime }}
          停挂时间:{{ hostDetail.hospitalInfo.bookingRule?.stopTime }}
        </div>
        <div class="address">具体地址:{{ hostDetail.hospitalInfo.hospital?.param.fullAddress }}</div>
        <div class="road">规划路线:{{ hostDetail.hospitalInfo.hospital?.route }}</div>
        <div class="close">退号时间:就诊前一工作日{{ hostDetail.hospitalInfo.bookingRule?.quitTime }}时间取消</div>
        <div class="rule">医院预约规则:</div>
        <div v-for="(item,index) in hostDetail.hospitalInfo.bookingRule?.rule" :key="index" class="ruleInfo">
          {{ item }}
        </div>
      </div>
    </div>
